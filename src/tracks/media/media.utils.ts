...
export function getSilenceArrayBuffer(
	...
	console.log(
		'max',
		channelData.reduce((max, val) => Math.max(max, val), 0),
	);
	console.log(
		'min',
		channelData.reduce((min, val) => Math.min(min, val), 0),
	);
	...
	if (silenceStart !== null) {
		...
		if (silenceDuration >= minSilenceChunks || silenceDuration == decibelValues.length) {
			silences.push({
				start: (silenceStart * sampleSize) / audioBuffer.sampleRate + offsetSeconds,
				stop: channelData.length / audioBuffer.sampleRate + offsetSeconds,
			});
		}
	}
